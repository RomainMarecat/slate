...
export class CartListService {
  ...
  deleteCartItem(mono: Mono, cartItem: CartItem): Observable<Cart> {
    ...
    this.deleteFromMonoCarts(cart, mono, cartItem);

    localStorage.setItem('cart', JSON.stringify(cart));
    return of(cart);
  }

  deleteFromMonoCarts(cart: Cart, mono: Mono, cartItem: CartItem) {
    let monoCart: MonoCart;
    let j = 0;

    cart.mono_carts.forEach((coach: MonoCart) => {
      if (coach.mono && coach.mono.user_id
        && coach.mono.user_id === mono.user_id
        && coach.cart_items && coach.cart_items.length > 0) {
        this.deleteFromCartItems(coach, cartItem, j);
        monoCart = coach;
      }
  ...
  deleteFromCartItems(coach: MonoCart, cartItem, j: number) {
    let idx = 0;
